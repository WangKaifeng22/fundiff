...
from model_utils import create_train_step, create_encoder_step, create_eval_step, create_autoencoder_state
...
def train_and_evaluate(config: ml_collections.ConfigDict):
    ...
    for epoch in range(10000):
        ...
        for batch in train_loader:
            ...
            if config.training.random_resolution:
                ...
                downsample_factors = jnp.array([1, 2, 4, 8])
                random_downsample = jax.random.choice(key1, downsample_factors)

                x, y = batch[0], batch[1]
                x = x[:, ::random_downsample, ::random_downsample]
                batch = (x, y)
    ...
    # Save latent outputs of encoder
    encoder_step = create_encoder_step(encoder, mesh)
    test_loader = create_dataloader(train_dataset,
    ...
    for batch in tqdm(test_loader):
        ...
        z = encoder_step(state.params[0], batch)
        encoder_output_list.append(z)

    encoder_outputs = np.concatenate(encoder_output_list, axis=0)
    save_path = os.path.join(os.getcwd(), job_name, "encoder_outputs.npy")
    np.save(save_path, encoder_outputs)
    print(f"Saved encoder outputs to {save_path}")
