...
def train_and_evaluate(config: ml_collections.ConfigDict):
    ...
    for epoch in range(10000):
        ...
        for batch in train_loader:
            ...
            batch = batch[..., :2]  # Only use the first two channels (u, v)
            ...
            state, loss, loss_u, loss_v = train_step(state, batch)
        ...
        if epoch % config.logging.log_interval == 0:
            ...
            end_time = time.time()
            log_dict = {"loss": loss, "lr": lr(step), "loss_u": loss_u, "loss_v": loss_v}

            if jax.process_index() == 0:
                wandb.log(log_dict, step)  # Log metrics to W&B
                print("step: {}, loss: {:.3e}, loss_u: {:.3e}, loss_v: {:.3e}, time: {:.3e}".format(step, loss, loss_u, loss_v, end_time - start_time))
