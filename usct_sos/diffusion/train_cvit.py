...
def train_and_evaluate(config: ml_collections.ConfigDict):
    ...
    for epoch in range(10000):
        ...
        for x in train_loader:
            ...
            x_branch, x_probe, x_aux, y = x
            if config.training.random_resolution:
                downsample_factors = jnp.array([1, 2, 5])
                random_downsample = int(jax.device_get(jax.random.choice(subkeys[0], downsample_factors)))
                y = jax.image.resize(
            ...
            coords, _, _, y_query = batch_parser.random_query(((x_branch, x_probe, x_aux), y), rng_key=subkeys[2])
            batch = (coords, y, y_query)

            batch = multihost_utils.host_local_array_to_global_array(
                batch, mesh, P("batch")
            )
            state, loss, loss_data, loss_res = train_step(state, batch, rng=subkeys[1])
