...
from data_utils import create_diffusion_dataset
...
def train_and_evaluate(config: ml_collections.ConfigDict):
    ...
    # Create dataloaders
    train_dataset, _ = create_diffusion_dataset(config)
    ...
    for epoch in range(10000):
        ...
        for batch in train_loader:
            ...
            z_p = encoder_step(fae_state.params[0], p_batch)
            z_sdf = encoder_step(fae_state.params[0], sdf_batch)

            z_1 = jnp.concatenate([z_p, z_sdf], axis=-1)  # (b, l, 2c)
            z_c = jnp.concatenate([z_u, z_v], axis=-1)    # (b, l, 2c)

            diffusion_batch, rng_key = get_diffusion_batch(rng_key, z1=z_1, c=z_c, use_conditioning=True)
